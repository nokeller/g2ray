...
def test_uniform_noise_ids_drops_host_error_cluster():
    # connect-style: one host returns identical 502 (len 932) for 20 paths -> drop
    rows = [(i, "connect.x.com", 502, 932) for i in range(20)]
    # plus real distinct results that must be KEPT
    rows += [(100, "api.x.com", 200, 50), (101, "api.x.com", 200, 73),
             (102, "api.x.com", 401, 18)]
    # a uniform 401 wall (auth-gated api surface) must be KEPT (signal)
    rows += [(200 + i, "auth.x.com", 401, 12) for i in range(20)]
    drop = set(ep.uniform_noise_ids(rows, threshold=12))
    assert drop == set(range(20))                     # only the 502 cluster
    assert 100 not in drop and 102 not in drop        # real results kept
    assert all((200 + i) not in drop for i in range(20))  # 401 wall kept


def test_uniform_noise_ids_keeps_small_clusters():
    # a host with 5 identical 500s is below threshold -> kept (could be real)
    rows = [(i, "h.x.com", 500, 100) for i in range(5)]
    assert ep.uniform_noise_ids(rows, threshold=12) == []
