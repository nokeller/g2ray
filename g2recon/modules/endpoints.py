...
# statuses where a large per-host cluster of byte-identical responses is a
# host-wide error / catch-all shell (backend 5xx, blanket 405/403/400) rather
# than many distinct endpoints. 401/422/200 are deliberately EXCLUDED: a uniform
# 401 wall is a real auth-gated API surface (prime bug-bounty signal), and JSON
# bodies of real 200/422 endpoints differ in length so they don't cluster.
PRUNE_STATUS = {400, 403, 405, 406, 410, 415, 500, 501, 502, 503, 504, 520, 521, 522, 530}


def uniform_noise_ids(rows, threshold: int = 12) -> list:
    """Given ``[(id, host, status_code, content_length), ...]`` return the ids
    of endpoint rows that belong to a large uniform (host, status, length)
    cluster on a noise status — i.e. a host answering many distinct paths with
    the identical error/catch-all response. Those are dropped to keep the
    endpoints view high-signal; genuine distinct endpoints are untouched."""
    from collections import defaultdict
    groups: dict[tuple, list] = defaultdict(list)
    for (rid, host, status, length) in rows:
        if status in PRUNE_STATUS:
            groups[(host, status, length)].append(rid)
    drop: list = []
