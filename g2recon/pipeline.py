...
from .db import (get_session, Target, Subdomain, Url, FileRecord, JsLink,
                 Reflection, OpenRedirect, Endpoint, Takeover, CorsFinding)
...
from .modules import (subdomains as m_sub, wayback as m_wb, downloader as m_dl,
                      ...
                      takeover as m_tko, corscheck as m_cors, scope as m_scope)

STEP_ORDER = ["subdomains", "wayback", "files", "endpoints", "params",
              "reflection", "openredirect", "livecheck", "takeover", "cors", "fuzz"]
...
class PipelineRunner:
    def __init__(self, target_id: int, job_id: int, options: dict,
        ...
        # per-run proxyless mode: build a client with an EMPTY proxy pool so every
        # request stays on the direct VPS IP (no proxy fallback at all), without
        # disturbing the globally-configured proxies for other runs.
        self.disable_proxy = bool(self.options.get("disable_proxy"))
        if self.disable_proxy:
            from .http_client import HttpClient
            self.client = HttpClient(proxies=[])
        else:
        ...
        # explicit program out-of-scope list (exact host / *.suffix). Active steps
        # (livecheck/fuzz/reflection/openredirect/endpoints/takeover/cors) skip these.
        self.oos = m_scope.parse_patterns(self.options.get("out_of_scope") or "")
    ...
    def _in_scope_hosts(self) -> list[str]:
        ...
        if self.oos:
            hosts = [h for h in hosts if not m_scope.host_excluded(h, self.oos)]
        return hosts
    ...
    def _all_urls(self, only_params=False, only_juicy=False) -> list[str]:
        ...
        urls = [r[0] for r in self.session.execute(q).all()]
        if self.oos:
            urls = [u for u in urls if not m_scope.host_excluded(util.host_of(u), self.oos)]
        return urls
    ...
    def run(self):
        ...
        self.session.commit()
        if self.disable_proxy:
            self.log("warn", "pipeline", "PROXYLESS run: direct VPS IP only — no proxy "
                     "fallback on WAF/rate-limit, waymore runs direct (Common Crawl "
                     "may be unreachable). Archive blocks will surface as real errors.")
    ...
    def _step_wayback(self):
        ...
        # refresh the master subdomains file so it includes hosts discovered in
        # the archive harvest (not just the enum sources from the subdomains step)
        self._write_subdomains_file()
    ...
    def _write_subdomains_file(self):
        hosts = sorted({h for (h,) in self.session.execute(
            select(Subdomain.host).where(Subdomain.target_id == self.target_id)).all()})
    ...
    def _step_endpoints(self):
        ...
        prober.run(cands, persist,
                   ...
                   workers=workers)
        self._prune_uniform_endpoints()
        self._write_endpoints_file()

    def _prune_uniform_endpoints(self, threshold: int = 12):
        """Remove host-wide uniform error/catch-all clusters (e.g. a backend
        that returned an identical 502 for 300 distinct paths) so the endpoints
        view stays high-signal. Keeps every distinct/auth-gated result."""
        rows = self.session.execute(
            select(Endpoint.id, Endpoint.host, Endpoint.status_code,
                   Endpoint.content_length)
            .where(Endpoint.target_id == self.target_id)).all()
        drop_ids = m_ep.uniform_noise_ids(rows, threshold=threshold)
        if not drop_ids:
            return
        from sqlalchemy import delete as _delete
        with self._lock:
            for i in range(0, len(drop_ids), 500):
                self.session.execute(_delete(Endpoint).where(
                    Endpoint.id.in_(drop_ids[i:i + 500])))
            self.session.commit()
        self.log("info", "endpoints", f"pruned {len(drop_ids)} uniform host-error/"
                 f"catch-all results (kept distinct + auth-gated endpoints)")
    ...
    # -- takeover ----------------------------------------------------------
    def _step_takeover(self):
        hosts = self._in_scope_hosts()
        workers = int(self.options.get("takeover_workers") or 40)
        scanner = m_tko.TakeoverScanner(self.client)

        def persist(rec):
            with self._lock:
                store.add_takeover(self.session, self.target_id, rec)
        scanner.run(hosts, persist,
                    lambda lvl, m: self.log(lvl, "takeover", m),
                    self.should_stop, workers=workers)
        self._write_takeover_file()

    def _write_takeover_file(self):
        rows = self.session.execute(
            select(Takeover.status, Takeover.severity, Takeover.host,
                   Takeover.provider, Takeover.cname, Takeover.evidence)
            .where(Takeover.target_id == self.target_id)
            .order_by(Takeover.severity, Takeover.host)).all()
        lines = [f"{st}\t{sev}\t{host}\t{prov}\t{cname}\t{ev}"
                 for (st, sev, host, prov, cname, ev) in rows]
        (self.dir / f"{self.slug}_takeover.txt").write_text("\n".join(lines) + "\n")
    ...
    def _step_cors(self):
        ...
        for (u,) in self.session.execute(
            ...
            if u and not (self.oos and m_scope.host_excluded(util.host_of(u), self.oos)):
                targets.add(u)
        cap = int(self.options.get("cors_max_urls")
                  if self.options.get("cors_max_urls") is not None else 6000)
        tl = list(targets)
        if cap and len(tl) > cap:
            self.log("warn", "cors", f"capping {len(tl)} -> {cap} cors targets")
            tl = tl[:cap]
        scanner = m_cors.CorsScanner(self.client)

        def persist(rec):
            with self._lock:
                store.add_corsfinding(self.session, self.target_id, rec)
        scanner.run(tl, persist,
                    lambda lvl, m: self.log(lvl, "cors", m),
                    self.should_stop, workers=int(self.options.get("cors_workers") or 25))
        self._write_cors_file()

    def _write_cors_file(self):
        rows = self.session.execute(
            select(CorsFinding.severity, CorsFinding.url, CorsFinding.acao,
                   CorsFinding.acac, CorsFinding.set_cookie, CorsFinding.note)
            .where(CorsFinding.target_id == self.target_id)
            .order_by(CorsFinding.severity)).all()
        lines = [f"{sev}\t{url}\tACAO={acao}\tACAC={acac}\tcookie={ck}\t{note}"
                 for (sev, url, acao, acac, ck, note) in rows]
        (self.dir / f"{self.slug}_cors.txt").write_text("\n".join(lines) + "\n")
